...
type Suit =
    ...
    static member internal ToInt(value) =
        match value with
        | Test.AvroMsg.Suit.SPADES -> 0
        | Test.AvroMsg.Suit.HEARTS -> 1
        | Test.AvroMsg.Suit.DIAMONDS -> 2
        | Test.AvroMsg.Suit.CLUBS -> 3
namespace rec Test.AvroMsg
...
type TestMessage =
    ...
    interface Avro.Specific.ISpecificRecord with
        ...
        member this.Get(pos: int) =
            ...
            | 7 -> box this.map
            | 8 -> box this.md5
            | 9 -> Test.AvroMsg.Suit.ToInt this.suit |> box
        ...
        member this.Put(pos: int, value: obj) =
            ...
            | 1, (:? int as x) -> this.GetType().GetProperty("num").SetMethod.Invoke (this, [| Some(x) |]) |> ignore
